...
describe('ResetPasswordWithRecoveryKeyVerified', () => {
  ...
  it('renders default content', async () => {
    ...
    expect(await screen.findByText(createRecoveryKeyText)).toBeInTheDocument();
    expect(await screen.findByText(continueToAccountText)).toBeInTheDocument();
  });

  it('renders default content when signed out', async () => {
    render(<ResetPasswordWithRecoveryKeyVerified isSignedIn={false} />);
    expect(await screen.findByText(singedOutText)).toBeInTheDocument();
    expect(await screen.findByText(createRecoveryKeyText)).toBeInTheDocument();
    expect(await screen.findByText(continueToAccountText)).toBeInTheDocument();
  });
  ...
  it('renders default content for sync service', async () => {
    ...
    expect(await screen.findByText(syncText)).toBeInTheDocument();
    expect(await screen.findByText(startBrowsingText)).toBeInTheDocument();
    expect(await screen.findByText(createRecoveryKeyText)).toBeInTheDocument();
    expect(await screen.findByText(continueToAccountText)).toBeInTheDocument();
  });
