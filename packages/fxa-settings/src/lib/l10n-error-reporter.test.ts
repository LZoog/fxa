...
describe('reportL10nError', () => {
  ...
  describe('before metrics are enabled', () => {
    ...
    it('never sends anything when the buffer is not flushed', () => {
      reportL10nError(new Error('No static asset mapping for l10n bundle'));
      resetL10nErrorReporter();
  ...
  describe('after metrics are enabled', () => {
    ...
    it('reports an error whose message has not been seen before', () => {
      reportL10nError(new Error('first failure'));
      reportL10nError(new Error('second failure'));
