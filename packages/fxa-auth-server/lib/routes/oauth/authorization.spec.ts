...
describe('/authorization POST consent write', () => {
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      jest.doMock('../../oauth/grant', () => ({
        validateRequestedGrant: jest.fn(async (_claims, _client, payload) => ({
          ...
          // A real ScopeSet, not a closure re-deriving from payload.scope, so
          // that a refactor mutating the grant's scopes in place is observable
          // rather than silently re-derived on each read.
  ...
  // user who never authorized Sync. Branch coverage lives in
  // ../../oauth/desktop-sync-consent-bandaid.spec.ts; these pin the wiring.
  ...
  // The bandaid must touch the consent ledger and nothing else. The grant
  // handed to generateCode is what becomes the auth code, and from there the
  // access and refresh tokens — if a refactor ever fed consentScopes into it,
  // Sync would silently break for these users.
  it('leaves the Sync scope on the grant when it drops it from the consent write', async () => {
    ...
    // Both halves of the divergence in one place: the ledger loses the Sync
    // scope while the grant that becomes the code — and from there the access
    // and refresh tokens — keeps it.
    const { scopes } = (oauthDB.recordSignInConsents as jest.Mock).mock
      .calls[0][0];
    expect([...scopes].sort()).toEqual([SMARTWINDOW_SCOPE, 'profile']);
  ...
  // The consent drop and the DAU tag are the same decision. /oauth/token never
  // sees `service=`, so the answer is stashed against the code's hash here and
  // read when the code is redeemed. Read side: ./token.spec.ts.
  describe('carrying the decision to /oauth/token', () => {
    ...
    it('writes nothing when the sign-in is Sync, so the token still counts', async () => {
      const authServerCacheRedis = { set: jest.fn().mockResolvedValue('OK') };
