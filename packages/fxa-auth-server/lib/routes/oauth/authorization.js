...
module.exports = ({ log, oauthDB, config, statsd, authServerCacheRedis }) => {
  ...
  // FXA-14263: hand the "don't count this as Sync DAU" decision to
  // /oauth/token, which never receives `service=` and so cannot make it
  // itself. The auth code is the only thing linking the two requests, so the
  // flag is stashed against the code's hash and expires with the code. Only
  // written when the answer is yes, so absence means "count it".
  //
  // Best-effort by design: if Redis is unavailable the event is counted, which
  // is exactly today's behaviour. Bookkeeping must not break sign-in.
  ...
  // Resolve the browser service this authorization is for. Lowercased so a
  // casing variant (service=VPN) does not silently land as service='' and
  // bypass the allowlist gate. When the URL omits service= (e.g. VPN cached
  // sign-in only sends client_id + scope=apps/vpn), recover it from a
  // canonical scope in the request so the row lands at service=<vpn> rather
  // than service=''. Only fires when exactly one canonical scope is present;
  // ambiguous multi-canonical requests fall back to ''.
  ...
  function shouldExcludeSyncDau(req, grant) {
    ...
    // short-circuiting here keeps a Desktop-only bandaid from resolving a
    // service on every web-RP authorization.
    ...
    return dropUnconsentedSyncScope({
      ...
      syncScope: oauthDB.getCanonicalScopeForService(OAuthNativeServices.Sync),
    }).droppedSyncScope;
  }
  ...
  async function recordAuthorizationRows(req, grant) {
    ...
    // the ledger when the user signed into a different browser service. The
    // grant and the tokens Desktop receives are untouched — see
    // ../../oauth/desktop-sync-consent-bandaid for why this exists and when it
    // can be deleted.
