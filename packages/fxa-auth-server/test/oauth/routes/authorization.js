...
const SERVICES_WITH_EMAIL_VERIFICATION_CLIENT = '6874a604b3fcf36d';
...
describe('/oauth/authorization POST (session token)', function () {
  describe('servicesWithEmailVerification enforcement', () => {
    it('rejects unverified sessions for clients in servicesWithEmailVerification', async () => {
      ...
      try {
        await sessionTokenRoute.config.handler(request);
        assert.fail('should have errored');
      } catch (err) {
